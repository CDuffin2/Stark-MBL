...
def Eigenvectors(L,Dim,H_1,T_0,T_1,gamma,w,V,navg):
    f = int(np.ceil(navg/Dim)) #disorder realisations
    ...
    for x in range(f):
        ...
        U[x] = np.linalg.eig(F)[1] #calculate eigenvectors
    return np.array(U)
...
def IPR(U,q,Dim,navg):
    ...
    I = np.zeros((f,Dim))
    for x in range(f):
        I[x] = np.sum(np.abs(U[x])**(2*q),axis=1)
    return I
...
def Tau(U,q_lst,nq,Dim,navg):
    ...
    for i in range(nq):
        ...
        #tau[i] = -np.log(np.mean(I))/np.log(Dim) #scaling exponent
        tau[i] = -np.log(np.exp(np.mean(np.log(I))))/np.log(Dim) #typical scaling exponent
    return tau
...
def Plots(tau,m,tau_ext,Dim_lst):
    ...
    ax = plt.gca()
    plt.annotate('(a)',(0.92,0.03),xycoords='axes fraction')
    ...
    plt.annotate('(b)',(0.92,0.03),xycoords='axes fraction')
...
def Parameters():
    ...
    navg = 10000 #total averaging (disorder*evecs)
    return L_lst,J,V,gamma,w,T_1,n,T_0,q_lst,navg,np.size(q_lst),np.size(L_lst)

start = time.time()
tau,m,tau_ext,Dim_lst = Main()
end = time.time()
